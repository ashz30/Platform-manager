...
def createVM(vm_name):
    ...
    print(f"Provisioned virtual machine {vm_result.name}, waiting for 60 secs for internal operations..")

    time.sleep(60)

    print(f"Restarting VM {VM_NAME}...")
    restartVM.restart(VM_NAME, RESOURCE_GROUP_NAME)
    return vm_result.name, ip_address_result.ip_address



def createVMAttachedResources(vm_name):
    VM_NAME = str(vm_name)
    USERNAME = "bpadmin"
    PASSWORD = "Blueprism@123"
    VNET_NAME = "AshishEasowSandbox-vnet"
